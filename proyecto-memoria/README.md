# Registro de préstamo de extensiones
...
## Contexto
La aplicación registra préstamos de extensiones eléctricas dentro de la escuela. Cada registro tiene:
- Nombre del solicitante
- Área o salón donde se ocupará
- Bloque
...
└── main
    ├── java
    │   ├── com/example/prestamosextensiones
    │   │   ├── controller
    │   │   │   └── MainController.java
    │   │   ├── model
    │   │   │   └── PrestamoExtension.java
    │   │   ├── repository
    │   │   │   └── PrestamoExtensionRepository.java
    │   │   ├── service
    │   │   │   └── PrestamoExtensionService.java
    │   │   └── Main.java
    │   └── module-info.java
    └── resources
        └── com/example/prestamosextensiones
...
Esto significa que:
- `txtNombreSolicitante`, `txtArea` y `cbBloque` sirven para capturar datos nuevos
...
1. Leer el contenido de `txtNombreSolicitante`, `txtArea` y `cbBloque`
2. Validar que no estén vacíos
3. Validar que no exista otro registro con el mismo nombre del solicitante
4. Crear un objeto `PrestamoExtension`
...
```text
María | Sala audiovisual | Bloque B
```

Paso 1:
- El usuario busca `María` o selecciona ese elemento del `ListView`
- Se cargan los datos en pantalla
- `nombreOriginal` guarda `María`
...
- el sistema busca el registro cuyo nombre original era `María`
- si lo encuentra, cambia sus datos

El resultado final será:
```text
María López | Laboratorio 2 | Laboratorio
```
...
- El área o salón no debe estar vacío
- El bloque debe seleccionarse
...
```text
src/main/resources/com/example/prestamosextensiones/main-view.fxml
```
